        ...
        <div class="sign_up_inner_cont">
            <form action="{{ url('/scheduleze/booking/update/'.$groupdata['id']) }}" method="post" name="appointment">
                ...
                <h3>Edit Appointment Details</h3>
                    ...
                    @else
                        @if(!empty($groupdata['type_pop']) && !empty($groupdata['size_pop']) || !empty($groupdata['type_pop']) && !empty($groupdata['age_pop']) || !empty($groupdata['size_pop']) && !empty($groupdata['age_pop']) || !empty($groupdata['size_pop']) && !empty($groupdata['type_pop']) || !empty($groupdata['age_pop']) && !empty($groupdata['type_pop']) || !empty($groupdata['age_pop']) && !empty($groupdata['size_pop']) || !empty($groupdata['type_pop']) && !empty($groupdata['size_pop']) && !empty($groupdata['age_pop']))
                            <div class="textfield_full">
                        @else
                    ...
                    @endif
                        <div class="three_col_cont">
                            <span class="signuplabel">
                                ...
                                {!! $groupdata['type_pop'] !!} @php !empty($groupdata['size_pop']) ? print_r($groupdata['size_pop']) : ''; @endphp {!! $groupdata['age_pop'] !!}
                            </span>
                        </div>
                    </div>
                    @if(!empty($groupdata['add_on_checkboxes']))
                        <div class="textfield_full">
                    ...
                    <div class="textfield_left">
                        ...
                        <?php $dayphone = preg_replace("/^(\d{3})(\d{3})(\d{4})$/", "$1-$2-$3", $groupdata['booking']->dayphone); ?>
                        <input type="text" name="Phone" id="phone" size="20" value="{{ $dayphone }}">
                    ...
                    <div class="textfield_right">
                        ...
                        <?php $homephone = preg_replace("/^(\d{3})(\d{3})(\d{4})$/", "$1-$2-$3", $groupdata['booking']->homephone); ?>
                        <input type="text" name="phone2" id="phone2" size="20" value="{{ $homephone }}"><!--<select name="phone2_name" class="smallselect">
                ...
                <div class="textfield_left">
                    <span class="signup_label_optional">Internal Notes:<br>
                        <textarea name="notes" rows="4" cols="70">{{ $groupdata['booking']->notes }}</textarea>
                    </span>
                </div>
                <div class="textfield_right">
                    <span class="signup_label_optional">Client Notes:<br>
                        <textarea name="user_notes" rows="4" cols="70">{{ $groupdata['booking']->user_notes }}</textarea>
                    </span>
                </div>
                <div class="textfield_full">
                    <input type="checkbox" name="send_confirm" value="1">
                    <span class="note">Send Confirmation Email</span>
                </div>
                <div class="textfield_full">
                    <input type="checkbox" name="send_agreement" value="1">
                    <span class="note">Send Contract/Agreement Email</span>
                </div>
                <div class="textfield_full">
